...
import { Settings, Zap, Activity, TableProperties, FileText } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 gap-3">
          ...
          >
            ...
            <div>
              ...
              <div className="text-xs opacity-70 mt-1">Latest Generation. Best balance of speed and high accuracy.</div>
            </div>
          </button>

          <button
            onClick={() => handleModelChange(ModelType.FLASH_1_5)}
            disabled={isAnalyzing}
            className={`flex items-start gap-3 p-3 rounded-xl border transition-all text-left ${
              config.model === ModelType.FLASH_1_5
                ? 'bg-blue-500/10 border-blue-500 text-slate-100 ring-1 ring-blue-500/50'
                : 'bg-slate-900 border-slate-700 text-slate-400 hover:border-slate-600'
            }`}
          >
            <Zap className={`w-5 h-5 mt-0.5 ${config.model === ModelType.FLASH_1_5 ? 'text-blue-400' : 'text-slate-500'}`} />
            <div>
              <div className="font-medium">Gemini 1.5 Flash</div>
              <div className="text-xs opacity-70 mt-1">Legacy High Speed. Use this for simpler tasks.</div>
            </div>
          </button>
        </div>
      </div>
